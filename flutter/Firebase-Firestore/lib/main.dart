  ...
  Widget _buildListItem(BuildContext context, DocumentSnapshot info) {
    return ListTile(
      ...
      title: Row(
        children: [
          Expanded(
            child: Text(info.data["name"],
                style: TextStyle(
                    fontWeight: (info?.data["especial"] != null &&
                            info?.data["especial"])
                        ? FontWeight.bold
                        : FontWeight.normal)),
          ),
      ...
      trailing: Row(
        ...
        children: [
          Container(
            ...
            child: Text(info.data["votes"].toString()),
          ),
          InkWell(
            onTap: () {
              info.reference.updateData({"votes": info.data["votes"] + 1});
              info.reference.setData({
                "especial": true,
                "${firebaseUser.email}":
                    (info.data["${firebaseUser.email}"] ?? 0) + 1
              }, merge: true);
              final String uid = firebaseUser.email;
            },
            child: Container(width: 50, child: Icon(Icons.arrow_upward)),
          ),
          InkWell(
            onTap: () {
              info.reference.delete();
            },
            child: Container(width: 30, child: Icon(Icons.delete)),
          )
        ],
      ),
    );
